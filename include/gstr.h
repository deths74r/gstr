 ...
 * using binary search.
 */
static int unicode_range_contains(uint32_t codepoint, const struct unicode_range *ranges, int count)
{
	int low = 0;
	int high = count - 1;
	while (low <= high) {
 ...
 */
static int is_grapheme_break(enum gcb_property prev_prop, enum gcb_property curr_prop, int ri_count, int in_ext_pict, uint32_t curr_cp, int incb_state)
{
	/* GB3: CR × LF */
 ...
 */
static int gstr_cmp_grapheme(const char *a, size_t a_len, const char *b, size_t b_len)
{
 ...
 */
static int gstr_cmp_grapheme_icase(const char *a, size_t a_len, const char *b, size_t b_len)
{
	size_t min_len = a_len < b_len ? a_len : b_len;
 ...
 */
static int gstr_grapheme_in_set(const char *g, size_t g_len, const char *set, size_t set_len)
{
	int offset = 0;
 ...
 */
static inline const char *gstrat(const char *s, size_t byte_len, size_t grapheme_n, size_t *out_len)
{
	if (!s || byte_len == 0)
		return NULL;
 ...
 */
static inline int gstrncmp(const char *a, size_t a_len, const char *b, size_t b_len, size_t n)
{
	if (n == 0)
		return 0;
	if (!a && !b)
 ...
 * Only ASCII characters are case-folded.
 */
static inline int gstrncasecmp(const char *a, size_t a_len, const char *b, size_t b_len, size_t n)
{
	if (n == 0)
		return 0;
	if (!a && !b)
...
static inline int gstrstartswith(const char *s, size_t s_len, const char *prefix, size_t prefix_len)
{
	if (!s || !prefix)
		return 0;
	if (prefix_len == 0)
 ...
 */
static inline int gstrendswith(const char *s, size_t s_len, const char *suffix, size_t suffix_len)
{
	if (!s || !suffix)
		return 0;
	if (suffix_len == 0)
...
static inline const char *gstrchr(const char *s, size_t len, const char *grapheme, size_t g_len)
{
 ...
 */
static inline const char *gstrrchr(const char *s, size_t len, const char *grapheme, size_t g_len)
{
	if (!s || len == 0 || !grapheme || g_len == 0)
		return NULL;
 ...
 * Returns a pointer to the match, or NULL if not found.
 */
static inline const char *gstrstr(const char *haystack, size_t h_len, const char *needle, size_t n_len)
{
	if (!haystack || !needle)
		return NULL;
	if (n_len == 0)
 ...
 * Returns a pointer to the match, or NULL if not found.
 */
static inline const char *gstrrstr(const char *haystack, size_t h_len, const char *needle, size_t n_len)
{
	if (!haystack || !needle)
		return NULL;
	if (n_len == 0)
 ...
 */
static inline const char *gstrcasestr(const char *haystack, size_t h_len, const char *needle, size_t n_len)
{
	if (!haystack || !needle)
		return NULL;
	if (n_len == 0)
 ...
 */
static inline const char *gstrpbrk(const char *s, size_t len, const char *accept, size_t a_len)
{
	if (!s || len == 0 || !accept || a_len == 0)
		return NULL;
...
static inline size_t gstrsub(char *dst, size_t dst_size, const char *src, size_t src_len, size_t start_grapheme, size_t count)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 */
static inline size_t gstrncpy(char *dst, size_t dst_size, const char *src, size_t src_len, size_t n)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 */
static inline size_t gstrncat(char *dst, size_t dst_size, const char *src, size_t src_len, size_t n)
{
	if (!dst || dst_size == 0)
		return 0;
...
static inline const char *gstrsep(const char **stringp, size_t *lenp, const char *delim, size_t d_len, size_t *tok_len)
{
 ...
 */
static inline size_t gstrreplace(char *dst, size_t dst_size, const char *src, size_t src_len, const char *old_str, size_t old_len, const char *new_str, size_t new_len)
{
	if (!dst || dst_size == 0)
		return 0;
...
static inline size_t gstrellipsis(char *dst, size_t dst_size, const char *src, size_t src_len, size_t max_graphemes, const char *ellipsis, size_t ellipsis_len)
{
	if (!dst || dst_size == 0)
		return 0;
...
static inline size_t gstrfill(char *dst, size_t dst_size, const char *grapheme, size_t g_len, size_t count)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 */
static inline size_t gstrlpad(char *dst, size_t dst_size, const char *src, size_t src_len, size_t width, const char *pad, size_t pad_len)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 * If src is already wide enough, copies it unchanged.
 */
static inline size_t gstrrpad(char *dst, size_t dst_size, const char *src, size_t src_len, size_t width, const char *pad, size_t pad_len)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 */
static inline size_t gstrpad(char *dst, size_t dst_size, const char *src, size_t src_len, size_t width, const char *pad, size_t pad_len)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 */
static inline size_t gstr_grapheme_width(const char *s, size_t byte_len, int offset, int next)
{
	/* Scan grapheme for special sequences */
	int has_zwj = 0;
	int regional_count = 0;
 ...
 */
static inline size_t gstrwtrunc(char *dst, size_t dst_size, const char *src, size_t src_len, size_t max_cols)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 */
static inline size_t gstrwlpad(char *dst, size_t dst_size, const char *src, size_t src_len, size_t target_cols, const char *pad, size_t pad_len)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 * Truncates source if it exceeds target. Column-width aware.
 */
static inline size_t gstrwrpad(char *dst, size_t dst_size, const char *src, size_t src_len, size_t target_cols, const char *pad, size_t pad_len)
{
	if (!dst || dst_size == 0)
		return 0;
 ...
 * Truncates source if it exceeds target. Column-width aware.
 */
static inline size_t gstrwpad(char *dst, size_t dst_size, const char *src, size_t src_len, size_t target_cols, const char *pad, size_t pad_len)
{
	if (!dst || dst_size == 0)
		return 0;
