...
#define EAW_WIDE_COUNT (sizeof(EAW_WIDE_RANGES) / sizeof(EAW_WIDE_RANGES[0]))
...
#define EXTENDED_PICTOGRAPHIC_COUNT                                            \
	(sizeof(EXTENDED_PICTOGRAPHIC_RANGES) /                                \
	 sizeof(EXTENDED_PICTOGRAPHIC_RANGES[0]))
...
#define UNICODE_PUNCT_SYMBOL_COUNT                                             \
	(sizeof(UNICODE_PUNCT_SYMBOL_RANGES) /                                 \
	 sizeof(UNICODE_PUNCT_SYMBOL_RANGES[0]))
...
static inline enum gcb_property get_gcb(uint32_t cp)
{
	...
	if (cp >= HANGUL_SBASE && cp <= HANGUL_SEND) {
		return ((cp - HANGUL_SBASE) % HANGUL_TCOUNT == 0) ? GCB_LV
								  : GCB_LVT;
	}
...
{
	return unicode_range_contains(cp, INCB_CONSONANTS,
				      INCB_CONSONANT_COUNT);
}
				    ...
				    int incb_state)
{
	...
	/* GB4: (Control | CR | LF) ÷ */
	if (prev_prop == GCB_CONTROL || prev_prop == GCB_CR ||
	    prev_prop == GCB_LF) {
		return 1;
	}

	/* GB5: ÷ (Control | CR | LF) */
	if (curr_prop == GCB_CONTROL || curr_prop == GCB_CR ||
	    curr_prop == GCB_LF) {
		return 1;
	}
	...
	/* GB6: L × (L | V | LV | LVT) */
	if (prev_prop == GCB_L &&
	    (curr_prop == GCB_L || curr_prop == GCB_V || curr_prop == GCB_LV ||
	     curr_prop == GCB_LVT)) {
		return 0;
	}
	...
	/* GB8: (LVT | T) × T */
	if ((prev_prop == GCB_LVT || prev_prop == GCB_T) &&
	    curr_prop == GCB_T) {
		return 0;
	}
...
static inline int utf8_cpwidth(uint32_t codepoint)
{
	...
	if (unicode_range_contains(codepoint, ZERO_WIDTH_RANGES,
				   ZERO_WIDTH_COUNT)) {
		return 0;
	}
...
{
	return unicode_range_contains(codepoint, ZERO_WIDTH_RANGES,
				      ZERO_WIDTH_COUNT);
}
 ...
 */
static inline size_t utf8_next_grapheme(const char *text, size_t length,
					size_t offset)
{
	if (!text || offset >= length) {
		return length;
	...
	while (next_offset < length) {
		uint32_t curr_cp;
		bytes = utf8_decode(text + next_offset, length - next_offset,
				    &curr_cp);
		enum gcb_property curr_prop = get_gcb(curr_cp);

		if (is_grapheme_break(prev_prop, curr_prop, ri_count,
 ...
 */
static inline size_t utf8_prev_grapheme(const char *text, size_t length,
					size_t offset)
{
	if (!text || length == 0 || offset == 0) {
		return 0;
 ...
 */
static inline size_t utf8_truncate(const char *text, size_t length,
				   size_t max_cols)
{
	if (!text) {
		return 0;
 ...
 */
static inline int gstr_cmp_grapheme_icase(const char *a, size_t a_len,
 ...
 */
static inline int gstr_grapheme_in_set(const char *g, size_t g_len,
				       const char *set, size_t set_len)
{
	...
	while (offset < set_len) {
		...
		if (grapheme_len == g_len &&
		    memcmp(g, set + offset, g_len) == 0) {
			return 1;
		}
		offset = next;
	}
			  ...
			  size_t b_len)
{
	...
	while (a_off < a_len && b_off < b_len) {
		...
		int cmp =
			gstr_cmp_grapheme(a + a_off, a_glen, b + b_off, b_glen);
		if (cmp != 0)
			return cmp;
			      ...
			      size_t b_len)
{
	...
	while (a_off < a_len && b_off < b_len) {
		...
		int cmp = gstr_cmp_grapheme_icase(a + a_off, a_glen, b + b_off,
						  b_glen);
		if (cmp != 0)
			return cmp;
				 ...
				 const char *prefix, size_t prefix_len)
{
	...
	while (p_off < prefix_len) {
		...
		if (s_glen != p_glen ||
		    memcmp(s + s_off, prefix + p_off, s_glen) != 0)
			return 0;
			       ...
			       size_t suffix_len)
{
	...
	while (suf_off < suffix_len) {
		...
		size_t s_next = utf8_next_grapheme(s, s_len, s_off);
		size_t suf_next =
			utf8_next_grapheme(suffix, suffix_len, suf_off);

		size_t s_glen = (size_t)(s_next - s_off);
		size_t suf_glen = (size_t)(suf_next - suf_off);

		if (s_glen != suf_glen ||
		    memcmp(s + s_off, suffix + suf_off, s_glen) != 0)
			return 0;
				  ...
				  const char *grapheme, size_t g_len)
{
	...
	while (offset < len) {
		...
		if (grapheme_len == g_len &&
		    memcmp(s + offset, grapheme, g_len) == 0) {
			return s + offset;
		}
				  ...
				  const char *needle, size_t n_len)
{
	...
	while (h_off < h_len) {
		...
		while (matched_graphemes < needle_graphemes) {
			...
			size_t h_next =
				utf8_next_grapheme(haystack, h_len, h_pos);
			size_t n_next =
				utf8_next_grapheme(needle, n_len, n_pos);

			size_t h_glen = (h_next - h_pos);
			size_t n_glen = (n_next - n_pos);

			if (h_glen != n_glen ||
			    memcmp(haystack + h_pos, needle + n_pos, h_glen) !=
				    0) {
				match = 0;
				break;
			}
				      ...
				      const char *needle, size_t n_len)
{
	...
	while (h_off < h_len) {
		...
		while (matched_graphemes < needle_graphemes) {
			...
			if (gstr_cmp_grapheme_icase(haystack + h_pos, h_glen,
						    needle + n_pos,
						    n_glen) != 0) {
				match = 0;
				break;
			     ...
			     size_t a_len)
{
	...
	while (offset < len) {
		...
		if (!gstr_grapheme_in_set(s + offset, grapheme_len, accept,
					  a_len)) {
			break;
		}
			      ...
			      size_t r_len)
{
	...
	while (offset < len) {
		...
		if (gstr_grapheme_in_set(s + offset, grapheme_len, reject,
					 r_len)) {
			break;
		}
			     ...
			     size_t count)
{
	...
	if (bytes_to_copy >= dst_size) {
		...
		while (fit_offset < start_offset + bytes_to_copy) {
			size_t next =
				utf8_next_grapheme(src, src_len, fit_offset);
				  ...
				  size_t *tok_len)
{
	...
	while (offset < len) {
		...
		if (gstr_grapheme_in_set(start + offset, grapheme_len, delim,
					 d_len)) {
			if (tok_len)
				*tok_len = offset;
			*stringp = start + next;
	...
	while (remaining > 0) {
		...
		if (!found) {
			...
			if (written + to_copy >= dst_size) {
				...
				while (fit_offset < to_copy &&
				       fit_offset < max_bytes) {
					size_t next = utf8_next_grapheme(
						pos, remaining, fit_offset);
					if (next <= max_bytes) {
						last_complete = next;
					} else {
						break;
		...
		if (before_len > 0) {
			if (written + before_len >= dst_size) {
				...
				while (fit_offset < before_len &&
				       fit_offset < max_bytes) {
					size_t next = utf8_next_grapheme(
						pos, before_len, fit_offset);
					if (next <= max_bytes) {
						last_complete = next;
					} else {
						break;
		...
		if (new_str && new_len > 0) {
			if (written + new_len >= dst_size) {
				...
				while (fit_offset < new_len &&
				       fit_offset < max_bytes) {
					size_t next = utf8_next_grapheme(
						new_str, new_len, fit_offset);
					if (next <= max_bytes) {
						last_complete = next;
					} else {
						break;
				  ...
				  const char *ellipsis, size_t ellipsis_len)
{
	...
	if (ellipsis_graphemes >= max_graphemes) {
		return gstrncpy(dst, dst_size, ellipsis, ellipsis_len,
				max_graphemes);
	}
	...
	if (src && src_len > 0) {
		...
		if (to_copy >= dst_size) {
			...
			while (offset < to_copy && offset < dst_size - 1) {
				size_t next = utf8_next_grapheme(src, src_len,
								 offset);
				if (next < dst_size) {
					last_complete = next;
				} else {
					 ...
					 size_t offset, size_t next)
{
	...
	while (cp_offset < next) {
		uint32_t cp;
		size_t cp_bytes =
			utf8_decode(s + cp_offset, byte_len - cp_offset, &cp);
		if (cp_bytes == 0)
			break;
		if (cp == 0x200D) {
			has_zwj = 1;
			       ...
			       const char *pad, size_t pad_len)
{
	...
	if (src_width >= target_cols) {
		return src ? gstrwtrunc(dst, dst_size, src, src_len,
					target_cols)
			   : 0;
	}
