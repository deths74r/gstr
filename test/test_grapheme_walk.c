...
static void walk_graphemes(const char *label, const char *text)
{
	...
	while (offset < len) {
		...
		printf("  [%2d] bytes %3d..%-3d (%2d bytes)  \"", grapheme,
		       offset, next - 1, span);
		...
		while (cp_offset < next) {
			uint32_t cp;
			int cp_bytes = utf8_decode(text + cp_offset,
						   len - cp_offset, &cp);
			printf(" ");
			print_cp(cp);
			if (cp == 0xFFFD) {
...
static void walk_file(const char *path)
{
	...
	while (fgets(line, sizeof(line), f)) {
		...
		while (offset < len) {
			...
			while (cp_off < next) {
				uint32_t cp;
				int cb = utf8_decode(line + cp_off,
						     len - cp_off, &cp);
				if (cp == 0xFFFD)
					has_replacement = 1;
				if (cb == 0)
		...
		if (mismatch || has_replacement || has_skip) {
			printf(" L%-3d graphemes: walk=%d gstrlen=%zu", lineno,
			       walk_count, glen);
			if (mismatch)
...
int main(int argc, char **argv)
{
	...
	const char *decomposed =
		"cafe\xCC\x81 re\xCC\x81sume\xCC\x81 nai\xCC\x88ve";
	...
	const char *boundary =
		"★♠♣♥♦"; /* U+2605, U+2660, U+2663, U+2665, U+2666 */
...
* ♠+ZWJ+♠ = 1 grapheme (GB11 applies). */
	...
	/* Test 5: Check is_extended_pictographic directly for known values */
	{
		...
		struct {
			...
			{0x2666, "♦ BLACK DIAMOND",
			 1}, /* Extended_Pictographic=Yes */
			{0x2605, "★ BLACK STAR",
			 0}, /* Extended_Pictographic=No */
			{0x1F0A1, "🂡 PLAYING CARD ACE",
			 0}, /* Extended_Pictographic=No */
		...
		for (int i = 0; i < num; i++) {
			...
			const char *status =
				(got == checks[i].expect) ? "OK" : "WRONG";
			printf(" %s U+%04X %-25s got=%d expected=%d\n", status,
			       checks[i].cp, checks[i].name, got,
			       checks[i].expect);
			if (got != checks[i].expect)
				bugs++;
		}
		if (bugs > 0) {
   ...
   * ================================================================ */
	...
	struct {
		...
		{"\xe2\x98\x85", 0x2605, "BLACK STAR",
		 1}, /* ea=A, terminals: 1 */
		{"\xe2\x99\xa0", 0x2660, "BLACK SPADE SUIT",
		 1}, /* ea=A, terminals: 1 */
		{"\xe2\x99\xa3", 0x2663, "BLACK CLUB SUIT",
		 1}, /* ea=A, terminals: 1 */
		{"\xe2\x99\xa5", 0x2665, "BLACK HEART SUIT",
		 1}, /* ea=A, terminals: 1 */
		{"\xe2\x99\xa6", 0x2666, "BLACK DIAMOND SUIT",
		 1}, /* ea=A, terminals: 1 */
		{"\xf0\x9f\x82\xa1", 0x1F0A1, "PLAYING CARD ACE SPADES",
		 1}, /* ea=N, width 1 */
		{"\xf0\x9f\x82\xa2", 0x1F0A2, "PLAYING CARD TWO SPADES",
		 1}, /* ea=N, width 1 */
	...
	for (int i = 0; i < num_checks; i++) {
		...
		printf("  %-6s U+%04X  %-24s %-12d %-12d %s\n",
		       width_checks[i].ch, width_checks[i].cp,
		       width_checks[i].name, got, expect, match);
	}
	...
	struct {
		...
		{0x2605, "*", 1},  {0x0020, " ", 1},  {0x2660, "S", 1},
		{0x0020, " ", 1},  {0x2663, "C", 1},  {0x0020, " ", 1},
		{0x2665, "H", 1},  {0x0020, " ", 1},  {0x2666, "D", 1},
		{0x0020, " ", 1},  {0x1F0A1, "A", 1}, {0x0020, " ", 1},
		{0x1F0A2, "2", 1},
	};
	...
	for (int i = 0; i < drift_len; i++) {
		...
		printf("  %-6s  U+%04X    %-10d  %-10d  %-+10d",
		       drift_seq[i].ch, drift_seq[i].cp, editor_col,
		       terminal_col, drift);
		if (drift != prev_drift)
			printf("  <-- cursor jumps");
		printf("\n");
