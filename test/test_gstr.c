...
#define ASSERT_EQ_SIZE(a, b)                                                       \
	do {                                                                       \
		if ((a) != (b)) {                                                  \
			printf(" FAIL\n    Expected %zu, got %zu\n    at %s:%d\n", \
			       (size_t)(b), (size_t)(a), __FILE__, __LINE__);      \
			tests_failed++;                                            \
			return;                                                    \
		}                                                                  \
	} while (0)
...
TEST(gstrlen_ascii)
{
	ASSERT_EQ_SIZE(gstrlen(ASCII, ASCII_LEN), 5);
}
...
TEST(gstrlen_null)
{
	ASSERT_EQ_SIZE(gstrlen(NULL, 0), 0);
}
...
TEST(gstroff_mixed)
{
	...
	ASSERT_EQ_SIZE(gstroff(MIXED, MIXED_LEN, 3),
		       3); /* wave+skin (8 bytes) */
...
TEST(gstrcmp_equal_ascii)
{
	ASSERT_EQ(gstrcmp("hello", 5, "hello", 5), 0);
}

TEST(gstrcmp_less_ascii)
{
	ASSERT(gstrcmp("abc", 3, "abd", 3) < 0);
}

TEST(gstrcmp_greater_ascii)
{
	ASSERT(gstrcmp("abd", 3, "abc", 3) > 0);
}
...
TEST(gstrcasecmp_different)
{
	ASSERT(gstrcasecmp("abc", 3, "ABD", 3) < 0);
}
...
TEST(gstrcat_buffer_limit)
{
	...
	ASSERT_EQ_SIZE(n,
		       7); /* "hi" (2) + "hello" (5) = 7, fits in 8 with null */
...
TEST(gstrcat_truncate)
{
	...
	size_t n = gstrcat(buf, sizeof(buf), "hello", 5);
	ASSERT_EQ_SIZE(n,
		       5); /* "hi" (2) + "hel" (3) = 5, fits in 6 with null */
...
TEST(gstrncasecmp_zero_n)
{
	ASSERT_EQ(gstrncasecmp("abc", 3, "xyz", 3, 0), 0);
}
...
TEST(gstrcount_single)
{
	ASSERT_EQ_SIZE(gstrcount("hello", 5, "ell", 3), 1);
}
...
TEST(gstrcount_emoji)
{
	...
	ASSERT_EQ_SIZE(
		gstrcount(s, WAVE_SKIN_LEN * 2 + 1, WAVE_SKIN, WAVE_SKIN_LEN),
		2);
}
...
{
	char buf[32];
	size_t n = gstrreplace(buf, sizeof(buf), "hello world", 11, "world", 5,
			       "there", 5);
	ASSERT_EQ_SIZE(n, 11);
...
TEST(gstrreplace_no_match)
{
	char buf[32];
	size_t n =
		gstrreplace(buf, sizeof(buf), "hello", 5, "xyz", 3, "abc", 3);
	ASSERT_EQ_SIZE(n, 5);
	ASSERT_STR_EQ(buf, "hello");
}
...
TEST(gstrendswith_empty)
{
	ASSERT(gstrendswith("hello", 5, "", 0) == 1);
}
...
TEST(gstrwidth_ascii)
{
	ASSERT_EQ_SIZE(gstrwidth("Hello", 5), 5);
}
...
TEST(gstrwidth_hangul_jamo)
{
   ...
   * 0 columns); leading jamo render as a spacing glyph and stay wide. */
	ASSERT_EQ_SIZE(gstrwidth("\xE1\x85\xA0", 3),
		       0); /* lone U+1160 medial */
	ASSERT_EQ_SIZE(gstrwidth("\xE1\x86\xA8", 3), 0); /* lone U+11A8 final */
	ASSERT_EQ_SIZE(gstrwidth("\xED\x9F\x8B", 3),
		       0); /* lone U+D7CB ext-B final */
	ASSERT_EQ_SIZE(gstrwidth("\xE1\x84\x92", 3),
		       2); /* lone U+1112 leading */
}
...
TEST(gstrwidth_hangul_decomposed)
{
   ...
   * 2 columns as the precomposed syllable. */
	ASSERT_EQ_SIZE(gstrwidth("\xE1\x84\x92\xE1\x85\xA1\xE1\x86\xAB", 9),
		       2); /* 한 L+V+T */
	ASSERT_EQ_SIZE(gstrwidth("\xE1\x84\x92\xE1\x85\xA1", 6),
		       2); /* 하 L+V */
}
...
TEST(grapheme_width_agrees_with_gstrwidth)
{
   ...
   * for the same string. */
	const char *samples[] = {ASCII,
				 ...
				 "a"
				 "\xF0\x9F\x98\x80"};
	const size_t sample_lens[] = {ASCII_LEN,     CAFE_DECOMPOSED_LEN,
				      FAMILY_LEN,    FLAG_CA_LEN,
				      WAVE_SKIN_LEN, 11};
...
TEST(gstrellipsis_emoji)
{
	...
	size_t n = gstrellipsis(buf, sizeof(buf), src, 3 * EMOJI_SIMPLE_LEN, 3,
				".", 1);
	/* Should be 1 emoji + "." */
	ASSERT(n > 0);
}
...
TEST(gstrfill_emoji)
{
	char buf[64];
	size_t n =
		gstrfill(buf, sizeof(buf), EMOJI_SIMPLE, EMOJI_SIMPLE_LEN, 3);
	ASSERT_EQ_SIZE(n, 3 * EMOJI_SIMPLE_LEN);
}
...
{
	char buf[64];
	size_t n = gstrpad(buf, sizeof(buf), "x", 1, 3, EMOJI_SIMPLE,
			   EMOJI_SIMPLE_LEN);
