...
TEST(ri_odd_count)
{
	...
	const char *three_ri =
		"\xF0\x9F\x87\xBA\xF0\x9F\x87\xB8\xF0\x9F\x87\xAC";
...
TEST(empty_gstrwidth)
{
	ASSERT_EQ_SIZE(gstrwidth("", 0), 0);
}
TEST(empty_gstrcmp)
{
...
TEST(empty_gstrstr)
{
	ASSERT_NOT_NULL(gstrstr("Hello", 5, "", 0));
}
TEST(empty_gstrrstr)
{
	ASSERT_NOT_NULL(gstrrstr("Hello", 5, "", 0));
}
...
TEST(null_gstrwidth)
{
	ASSERT_EQ_SIZE(gstrwidth(NULL, 0), 0);
}
TEST(null_gstrchr)
{
	ASSERT_NULL(gstrchr(NULL, 0, "A", 1));
}
TEST(null_gstrstr)
{
	ASSERT_NULL(gstrstr(NULL, 0, "A", 1));
}
TEST(null_gstrdup)
{
	ASSERT_NULL(gstrdup(NULL, 0));
}
