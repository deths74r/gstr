...
static void test_gstrsep_comprehensive(void)
{
	...
	/* With emoji delimiter */
	{
		...
		while (s) {
			const char *tok =
				gstrsep(&s, &len, EMOJI, EMOJI_LEN, &tok_len);
			if (tok)
				count++;
		}
		if (count == 3)
...
static void test_gstrrev_comprehensive(void)
{
	...
	/* Mixed ASCII and emoji: "A😀B" -> "B😀A" */
	{
		...
		if (n == src_len && buf[0] == 'B' &&
		    memcmp(buf + 1, EMOJI, EMOJI_LEN) == 0 &&
		    buf[1 + EMOJI_LEN] == 'A')
			passed++;
		else {
			failed++;
   ...
   */
	...
	if (n == HANGUL_LEN) {
		...
		if (memcmp(buf, HANGUL + 3, 3) == 0 &&
		    memcmp(buf + 3, HANGUL, 3) == 0)
			passed++;
		else {
			failed++;
...
static void test_gstrreplace_comprehensive(void)
{
	...
	/* Basic replacement */
	n = gstrreplace(buf, sizeof(buf), "hello world", 11, "world", 5,
			"there", 5);
	...
	/* Emoji replacement */
	{
		...
		n = gstrreplace(buf, sizeof(buf), src, 2 + EMOJI_LEN + 1, EMOJI,
				EMOJI_LEN, ":)", 2);
	...
	n = gstrreplace(buf, sizeof(buf), FAMILY, FAMILY_LEN,
			"\xF0\x9F\x91\xA9", 4, "X", 1);
	if (n == FAMILY_LEN && memcmp(buf, FAMILY, FAMILY_LEN) == 0)
		passed++;
	else {
		failed++;
...
static void test_stress_edge_cases(void)
{
	...
	/* Very long string of emoji */
	{
		char long_str[512];
		size_t pos = 0;
		for (int i = 0; i < 50 && pos + EMOJI_LEN < sizeof(long_str);
		     i++) {
