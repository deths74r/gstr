...
static struct test_string TEST_STRINGS[] = {
	...
	{"o_umlaut_decomposed", "o\xCC\x88", 3,
	 1},                                      /* o + combining diaeresis */
	...
	{"three_emoji", "\xF0\x9F\x98\x80\xF0\x9F\x98\x81\xF0\x9F\x98\x82", 12,
	 3},
	...
	{"complex_mixed",
	 "A\xCC\x81\xE4\xB8\xAD\xF0\x9F\x87\xBA\xF0\x9F\x87\xB8", 14,
	 3}, /* Á中🇺🇸 - 14 bytes */
	...
	/* Arabic */
	{"arabic", "\xD9\x85\xD8\xB1\xD8\xAD\xD8\xA8\xD8\xA7", 10,
	 5}, /* مرحبا */
...
static void test_gstrlen_vs_cpcount(void)
{
	...
	for (int i = 0; TEST_STRINGS[i].name != NULL; i++) {
		...
		if (grapheme_count <= (size_t)codepoint_count &&
		    codepoint_count >= 0) {
			PASS();
		} else {
			printf("  %-25s graphemes=%zu, codepoints=%d\n",
			       t->name, grapheme_count, codepoint_count);
			FAIL(t->name);
		}
	}
}
	...
	for (int i = 0; TEST_STRINGS[i].name != NULL; i++) {
		...
		if (end_offset == t->byte_len) {
			...
			printf("  %-25s gstroff(%zu)=%zu, expected %zu\n",
			       t->name, count, end_offset, t->byte_len);
			FAIL(t->name);
		}
	}
}
...
static void test_gstrsub_roundtrip(void)
{
	...
	for (int i = 0; TEST_STRINGS[i].name != NULL; i++) {
		...
		char buf[256];
		size_t written = gstrsub(buf, sizeof(buf), t->data, t->byte_len,
					 0, count);

		if (written == t->byte_len &&
		    memcmp(buf, t->data, t->byte_len) == 0) {
			PASS();
		} else {
			printf("  %-25s extract all: wrote %zu, expected %zu\n",
			       t->name, written, t->byte_len);
			FAIL(t->name);
		}
	}
}
...
static void test_gstrcpy_roundtrip(void)
{
	...
	for (int i = 0; TEST_STRINGS[i].name != NULL; i++) {
		...
		char buf[256];
		size_t written =
			gstrcpy(buf, sizeof(buf), t->data, t->byte_len);
...
static void test_gstrstr_comprehensive(void)
{
	...
	const char *hay2 =
		"Say hi \xF0\x9F\x91\x8B to me";  /* Say hi 👋 to me */
...
static void test_buffer_overflow_protection(void)
{
	...
	const char *emoji =
		"\xF0\x9F\x98\x80\xF0\x9F\x98\x81"; /* 😀😁 - 8 bytes */
