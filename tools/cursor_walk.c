...
#define utf8_next_grapheme_compat(text, len, off)                              \
	utf8_next_grapheme(text, (int)(len), (int)(off))
#define utf8_decode_compat(text, len, cp) utf8_decode(text, (int)(len), cp)
#define gstr_grapheme_width_compat(s, len, off, next)                          \
	0 /* not available in v1 */
#define HAS_GRAPHEME_WIDTH 0
#else
#define utf8_prev_grapheme_compat(text, len, off)                              \
	utf8_prev_grapheme(text, len, off)
#define utf8_next_grapheme_compat(text, len, off)                              \
	utf8_next_grapheme(text, len, off)
#define utf8_decode_compat(text, len, cp) utf8_decode(text, len, cp)
#define gstr_grapheme_width_compat(s, len, off, next)                          \
	gstr_grapheme_width(s, len, off, next)
#define HAS_GRAPHEME_WIDTH 1
#endif

/* GCB property name mapping */
static const char *GCB_NAMES[] = {"OTHER",
				  "CR",
				  "LF",
				  "CONTROL",
				  "EXTEND",
...
static const struct test_string TEST_STRINGS[] = {
	...
	TS("ZWJ: person + laptop",
	   "\xF0\x9F\xA7\x91\xE2\x80\x8D\xF0\x9F\x92\xBB", 0),
	...
	TS("Flags: odd RI count (5)",
	   ...
	   "\xF0\x9F\x87\xA9",
	   0),
	TS("Flags: single RI", "\xF0\x9F\x87\xBA", 0),
	...
	/* Indic Conjunct (GB9c) */
	TS("Devanagari: KA+VIRAMA+KA", "\xE0\xA4\x95\xE0\xA5\x8D\xE0\xA4\x95",
	   0),
	TS("Devanagari: triple conjunct",
	   "\xE0\xA4\x95\xE0\xA5\x8D\xE0\xA4\xB7\xE0\xA5\x8D\xE0\xA4\xA4", 0),
	TS("Bengali: ksha", "\xE0\xA6\x95\xE0\xA7\x8D\xE0\xA6\xB7", 0),

	/* Hangul Jamo */
	TS("Hangul L+V+T", "\xE1\x84\x80\xE1\x85\xA1\xE1\x86\xA8", 0),
	TS("Hangul LV+T (precomposed)", "\xEA\xB0\x80\xE1\x86\xA8", 0),

	/* Stacked Combining Marks */
	TS("Combining: e + 5 accents",
	   "e\xCC\x81\xCC\x81\xCC\x81\xCC\x81\xCC\x81", 0),
	TS("Combining: mixed marks", "e\xCC\x81\xCC\xA7\xCC\x88", 0),

	/* CR/LF */
	TS("CRLF sequence", "\r\n", 0),
	TS("CR alone then a then LF",
	   "\r"
	   "a\n",
	   0),
	...
	TS("Mixed: combining + CJK + emoji",
	   "e\xCC\x81\xE4\xB8\xAD\xF0\x9F\x98\x80", 0),
	...
	/* Full sentence */
	TS("Full sentence",
	   "Hello \xF0\x9F\x91\x8B\xF0\x9F\x8C\x8D world! \xE2\x9C\xA8", 0),
...
static void draw_screen(struct cursor_state *st)
{
	...
	size_t cur_end = (ts->length > 0)
				 ? utf8_next_grapheme_compat(
					   ts->data, ts->length, cur_start)
				 : 0;
	...
	printf("\033[1mgstr-cursor-walk\033[0m | Unicode %s | String %zu/%zu: \033[1m%s\033[0m",
	       GSTR_UNICODE_VERSION, st->string_idx + 1, (size_t)NUM_STRINGS,
	       ts->label);
	...
	while (off < ts->length) {
		size_t next =
			utf8_next_grapheme_compat(ts->data, ts->length, off);
	...
	move_to(7, 1);
	printf("  Byte offset:     %zu..%zu (%zu bytes)", cur_start, cur_end,
	       g_byte_len);
	move_to(8, 1);
	size_t gwidth = (size_t)gstr_grapheme_width_compat(ts->data, ts->length,
							   cur_start, cur_end);
	if (HAS_GRAPHEME_WIDTH)
		printf("  Display width:   %zu col%s", gwidth,
		       gwidth == 1 ? "" : "s");
	else
	...
	while (off < cur_end) {
		uint32_t cp;
		size_t cb =
			utf8_decode_compat(ts->data + off, cur_end - off, &cp);
		if (cb == 0)
			break;
		cp_count++;
		off += cb;
	...
	while (off < cur_end && detail_row < rows - 4) {
		...
		const char *prop_name =
			((size_t)prop <
			 sizeof(GCB_NAMES) / sizeof(GCB_NAMES[0]))
				? GCB_NAMES[prop]
				: "?";

		move_to(detail_row, 1);
		printf("    [%zu] U+%04X  %-18s  width=%d", cp_idx, cp,
		       prop_name, utf8_cpwidth(cp));

		if (st->show_bytes) {
			printf("  bytes:");
			for (size_t b = 0; b < cb; b++)
				printf(" %02X",
				       (unsigned char)ts->data[off + b]);
		}
	...
	if (st->show_verify && cur_end <= ts->length) {
		detail_row++;
		move_to(detail_row, 1);
		size_t roundtrip = utf8_prev_grapheme_compat(
			ts->data, ts->length,
			utf8_next_grapheme_compat(ts->data, ts->length,
						  cur_start));
		if (roundtrip == cur_start) {
			printf("  \033[32m\xe2\x9c\x93 fwd->bwd roundtrip OK (offset %zu)\033[0m",
			       roundtrip);
		} else {
	...
	char status[256];
	snprintf(status, sizeof(status), " Grapheme %zu/%zu | %zu bytes | %s",
...
static void advance_grapheme(struct cursor_state *st)
{
	...
	if (ts->length == 0)
		return;
	size_t next = utf8_next_grapheme_compat(ts->data, ts->length,
						st->byte_offset);
	if (next < ts->length) {
		st->byte_offset = next;
		st->grapheme_idx++;
...
{
	const struct test_string *ts = &TEST_STRINGS[st->string_idx];
	if (ts->length == 0 || st->byte_offset == 0)
		return;
	st->byte_offset = utf8_prev_grapheme_compat(ts->data, ts->length,
						    st->byte_offset);
	if (st->grapheme_idx > 0)
		st->grapheme_idx--;
}
...
static void run_interactive(void)
{
	if (!isatty(STDIN_FILENO)) {
		fprintf(stderr,
			"cursor_walk: interactive mode requires a terminal\n");
		exit(1);
	}
...
static int verify_string(const struct test_string *ts, int string_idx)
{
	...
	if (ok) {
		for (size_t i = 0; i + 1 < fwd_count; i++) {
			size_t next = utf8_next_grapheme_compat(
				ts->data, ts->length, fwd[i]);
			size_t back = utf8_prev_grapheme_compat(
				ts->data, ts->length, next);
			if (back != fwd[i]) {
				ok = 0;
				break;
			}
	...
	if (ok) {
		...
		for (size_t i = 0; i < fwd_count && i < 20; i++)
			printf(" %zu", fwd[i]);
		printf("\n       bwd(%zu):", bwd_count);
		for (size_t i = 0; i < bwd_count && i < 20; i++)
			printf(" %zu", bwd[i]);
		printf("\n");
		return 1;
	}
